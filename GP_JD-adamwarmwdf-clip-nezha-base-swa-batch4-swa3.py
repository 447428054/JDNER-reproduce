...
savedir = './saveGP-nezha-base-clip-1-swa3/10-adamwarmwdf-2e-5-batch32-10'
datadir = './data'
datatype = 'JD5'
bert_type = 'nezha'
model_scale = 'base'
optimizertype = 'adamwarmwdf'
is_flat = True
...
elif optimizertype == 'adamwarmwdf':
    total, warmsteps = calc_train_steps(32000, batch_size, epochs, warmup_proportion)
    AdamW = extend_with_weight_decay(Adam)
    AdamWLR = extend_with_piecewise_linear_lr(AdamW)
    AdamWLRDF = extend_with_parameter_wise_lr(AdamWLR)
    optimizer = AdamWLRDF(learning_rate=learning_rate,
                        weight_decay_rate=weight_decay,
                        lr_schedule={warmsteps: 1, total: 0.1},
                        clipvalue=1,
                        paramwise_lr_schedule={'Transformer': 10}
                        )
