...
savedir = './saveGP-nezha-base-clip-1-swa3-logloss/10-adamwarmw-2e-6-batch4'
datadir = './data'
datatype = 'JD5'
bert_type = 'nezha'
model_scale = 'base'
...
def multilabel_categorical_logcrossentropy(y_true, y_pred):
    """多标签分类的交叉熵
    说明：
    ...
    neg_loss = K.clip(neg_loss, 1e-7, 1e7)
    pos_loss = K.clip(pos_loss, 1e-7, 1e7)
    return K.log(neg_loss) + K.log(pos_loss)
...
def global_pointer_crossentropy(y_true, y_pred):
    ...
    return K.mean(multilabel_categorical_logcrossentropy(y_true, y_pred))
