...
savedir = './saveGP-nezha-base-clip-1-swa3-simcse/10-adamwarmw-2e-6-batch4-1'
datadir = './data'
datatype = 'JD5'
bert_type = 'nezha'
model_scale = 'base'
...
checkpoint_path = '/home/ytkj/root1/lizheng/2020July/CN_pretrained/NEZHA-Base/bert_model.ckpt'
dict_path = '/home/ytkj/root1/lizheng/2020July/CN_pretrained/NEZHA-Base/vocab.txt'
...
def multilabel_categorical_crossentropy_labelsmooth(y_true, y_pred, ls=0.2):
    """多标签分类的交叉熵
    说明：
    ...
    y_pred_pos += K.log(y_true_ls) + -1e12 * (1 - y_true)
    y_pred_neg += K.log(1 - y_true_ls) + -1e12 * y_true
...
def global_pointer_crossentropy_simcse(y_true, y_pred):
    """给GlobalPointer设计的交叉熵
    """
    bh = K.prod(K.shape(y_pred)[:2])
    y_true = K.reshape(y_true, (bh, -1))
    y_pred = K.reshape(y_pred, (bh, -1))
    loss_simcse = simcse_loss(y_true, y_pred)
    return K.mean(multilabel_categorical_crossentropy(y_true, y_pred)) + loss_simcse
...
model = build_transformer_model(config_path, checkpoint_path, model=bert_type, dropout_rate=0.3)
...
model.compile(
    loss=global_pointer_crossentropy_simcse,
    optimizer=optimizer,
