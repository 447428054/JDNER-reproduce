...
from models import GlobalPointerTagger
...
import os
from bert4keras.models import GlobalPointer
from keras.layers import *
from keras.utils import to_categorical
os.environ["CUDA_VISIBLE_DEVICES"] = ""

savedir = './saveGProbertastage2-pseudo8133fgmswa-multask-isless/10-adamwarmw-8e-6'
datadir = './data'
datatype = 'JD5pseudo-0.8133'
bert_type = 'bert'
model_scale = 'base'
...
# bert配置
config_path = '/home/root1/lizheng/pretrainModels/tensorflow/chinese/chinese_roberta_wwm_ext_L-12_H-768_A-12/bert_config.json'
checkpoint_path = '/home/root1/lizheng/pretrainModels/tensorflow/chinese/chinese_roberta_wwm_ext_L-12_H-768_A-12/bert_model.ckpt'
dict_path = '/home/root1/lizheng/pretrainModels/tensorflow/chinese/chinese_roberta_wwm_ext_L-12_H-768_A-12/vocab.txt'
...
for x in range(1, 55):
    if x not in [27, 45]:
        ...
        label_map['B-{}'.format(x)] = len(label_map)
        label_map['I-{}'.format(x)] = len(label_map)
...
lesstps = [29, 34, 31, 6, 30, 15, 49, 21, 47, 23, 20, 50, 46, 41, 43, 48, 19, 52, 33, 28, 32, 44, 25, 17, 42, 24, 53, 26, 35, 51]

class data_generator(DataGenerator):
    """数据生成器
    """
    def __iter__(self, random=False):
        batch_token_ids, batch_segment_ids, batch_labels, batch_seq_labels = [], [], [], []
        for is_end, d in self.sample(random):
            ...
            for _ in range(maxlen):
                seqlabels.append([0] * len(label_map))
            ...
            if flag:
                batch_seq_labels.append([1])
            else:
                batch_seq_labels.append([0]) # batch * 1

            batch_token_ids.append(token_ids)
            batch_segment_ids.append(segment_ids)
            batch_labels.append(labels[:, :len(token_ids), :len(token_ids)])
            # batch_seq_labels.append(seqlabels) # batch * 128
            if len(batch_token_ids) == self.batch_size or is_end:
                batch_token_ids = sequence_padding(batch_token_ids)
                batch_segment_ids = sequence_padding(batch_segment_ids)
                batch_labels = sequence_padding(batch_labels, seq_dims=3)
                # batch_seq_labels = sequence_padding(batch_seq_labels)
                lg = batch_token_ids.shape[1]
                batch_seq_labels = np.array(batch_seq_labels)
                yield [batch_token_ids, batch_segment_ids], [batch_labels, batch_seq_labels]
                batch_token_ids, batch_segment_ids, batch_labels, batch_seq_labels = [], [], [], []
...
def global_pointer_crossentropy_multask(y_true, y_pred):
    """给GlobalPointer设计的交叉熵
    """
    print(y_true.shape)
    print(y_pred.shape)
    ...
    bh = K.prod(K.shape(point_pred)[:2])
    point_true = K.reshape(point_true, (bh, -1))
    point_pred = K.reshape(point_pred, (bh, -1))
    return K.mean(multilabel_categorical_crossentropy(point_true, point_pred)) + K.mean(categorical_crossentropy(tag_true, tag_pred, from_logits=True))
...
clsoutput = Lambda(lambda x: x[:, 0], name='CLS-token')(model.output)
tagdense = Dense(1, name='tag_output', activation='sigmoid')
tagoutput = tagdense(clsoutput)
output = GlobalPointer(len(categories), 64)(model.output)

model = Model(model.input, [output, tagoutput])
model.summary()
...
if optimizertype == 'adam':
    ...
    total, warmsteps = calc_train_steps(46000, batch_size, epochs, warmup_proportion)
    AdamW = extend_with_weight_decay(Adam)
...
model.compile(
    loss={'global_pointer_1':global_pointer_crossentropy, 'tag_output': binary_crossentropy},
    loss_weights={'global_pointer_1': 1, 'tag_output': 0.1},
    optimizer=Adam(learning_rate),
    metrics={'global_pointer_1': [global_pointer_f1_score], 'tag_output': ['accuracy']}
)
...
class NamedEntityRecognizer(object):
    ...
    def recognize(self, text, threshold=0):
        ...
        scores = model.predict([token_ids, segment_ids])[0][0]
        scores[:, [0, -1]] -= np.inf
        scores[:, :, [0, -1]] -= np.inf
        entities = []
        escores = []
