...
import logging
import time
from bert4keras.optimizers import Adam, extend_with_piecewise_linear_lr, extend_with_parameter_wise_lr, extend_with_weight_decay,extend_with_lookahead
...
savedir = './saveGP-nezha-base-clip-1-swa3-unlabelaugtime20-traintest-lookaheadV2/10-adamwarmw-2e-6-batch4-epo4'
datadir = './data'
datatype = 'JD5'
bert_type = 'nezha'
model_scale = 'base'
optimizertype = 'adamwarmwlh'
is_flat = True
...
elif optimizertype == 'adamwarmwlh':
    total, warmsteps = calc_train_steps(32000, batch_size, epochs, warmup_proportion)
    AdamW = extend_with_weight_decay(Adam)
    AdamWLR = extend_with_piecewise_linear_lr(AdamW)
    AdamWLRLH = extend_with_lookahead(AdamWLR)
    optimizer = AdamWLRLH(learning_rate=learning_rate,
                        weight_decay_rate=weight_decay,
                        lr_schedule={warmsteps: 1, total: 0.1},
                        clipvalue=1
                        )
